...
autoketik = false;
prefixStatus = true;
targetpc = "6285942375691";
owner = "6283897539628";
fakeyoi = "irsyah";
HunterApi = "Ikyy69",
valkey = "rivalgans",
dapapi = "itsmevall",
nomorowner1 = "@6283897539628";
...
await itsmevall.updatePresence(from, Presence.composing)
    ...
    const ownerNumber = [`6282284568556@s.whatsapp.net`]
...
const reply = (teks) => {
			itsmevall.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:bgg,sourceUrl:`https://wa.me/62882015853719?text=Sv Bang`}}})
		}
      ...
      message: {
        productMessage: {
          product: {
            ...
            title: "UDAH COLI BLOM BG:V", //Kasih namalu
...
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-Bot\nN d y i e. (@Ndyie_Botz) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/6283800608785`,
                "mediaType": "10",
                "mediaUrl": `wa.me/6283800608785`,
                "thumbnailUrl": "https://k.top4top.io/p_20779elwn0.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/6283800608785`,
...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let _votes = JSON.parse(fs.readFileSync(`./lib/vote/${from}.json`));
        let fil = vote.map((v) => v.participant);
        let id_vote = sender ? sender : "62882015853719@s.whatsapp.net";
        if (fil.includes(id_vote)) {
          return mentions(
            "@" + sender.split("@")[0] + " Anda sudah vote",
            fil,
...
case 'menu':
       txtt =`Hai Kak ${pushname}\nPilih Opsi Dibawah Ini Ya.. `
               ...
               {buttonId:`${prefix}status`,buttonText:{displayText:'STATUS BOT🛠️'},type:1},
               {buttonId:`${prefix}mode`,buttonText:{displayText:'MODE BOT🤖'},type:1}]
...
InsTaGram = https://instagram.com/Rendy_nes
GiThuB = https://github.com/Ndyie
 
*Thanks Big Too🛐*
*-Allah Swt✨*
*-Ortuku👥*
*-MhankBarBar,Aqullz,ItsMeVall*
*- Ndyie Botz✨*`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
break
case 'iggw':
  reply('https://instagram.com/Rendy_nes')
break
      /*case "owner":
        sendKontak(from, owner, "ItsMeVallッ", "Sibuk");*/
case 'owner':
           case 'creator':
               ...
               buttonsMessage = { contentText: `${haibg}`, footerText: 'Kalo Mau Di Sv Chat Aja Kak🗿 Jangan sungkan chat yah🥰', buttons: buttons, headerType: 1 }
...
case "asupan3": // by itsmevall
        sendButMessage(from, `Hai Kak ${pushname} 🐤`, `Silahkan pilih Asupannya V.3 kak✨`, [
          ...
          },
          {
            ...
            buttonText: {
              displayText: `SUBSCRIBE CHANEL SAYA KACK🤗`,
            },
            type: 1,
          },
        ]);
...
itsmevall.sendMessage(from, stdout, text, term)
      ...
      default:
        ...
        if (hour_now >= '18:00' && hour_now <= '19:00') {
        	console.log(color('[Pesan Bot]', 'cyan'), color('Alhamdulillah, Dah buka kak, Puasanya dibatalin dulu kak, Sebelum makan jangan lupa baca doa kak, Setelah makan langsung sholat Maghrib ya kak', 'yellow'), color('(😊)', 'white'))
        }
        if (hour_now >= '19:00' && hour_now <= '20:00') {
           console.log(color('[Pesan Bot]', 'cyan'), color('Bentar lagi jam 8 kak, Yok kak main botnya buat nanti lagi, Sekarang siap-siap dulu buat Sholat Tarawih', 'yellow'), color('(😊)', 'white'))
           }
        if (hour_now >= '20:00' && hour_now <= '00:00') {
          ...
          if (hour_now >= '00:00' && hour_now <= '02:00') {
           console.log(color('[Pesan Bot]', 'cyan'), color('ManikBot ngantuk kak, tidur dulu ya kak', 'yellow'), color('(😴)', 'white'))
        }
